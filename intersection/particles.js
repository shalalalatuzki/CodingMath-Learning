window.onload = function() {
    ...
    function update() {
        ...
        var p0 = { x: particle.x, y: particle.y };
        particle.x += particle.vx;
        particle.y += particle.vy;

        context.fillRect(particle.x - 2, particle.y - 2, 4, 4);

        var p1 = { x: particle.x, y: particle.y };

        console.log("drawParticle");
        for (var i = 0; i < lines.length; i++) {
            var p2 = lines[i].p0,
                p3 = lines[i].p1;
            var intersect = segementIntersect(p0, p1, p2, p3);
            if (intersect) {
                context.beginPath();
    ...
    function segementIntersect(p0, p1, p2, p3) {
        var A1 = p1.y - p0.y,
            B1 = p0.x - p1.x,
            C1 = A1 * p0.x + B1 * p0.y,
            A2 = p3.y - p2.y,
